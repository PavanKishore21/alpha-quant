...
class AdvancedRiskManager:
    def __init__(self, lookback_period=42):
...
class OptimizedMLRankingSystem:
    def __init__(self):
        ...
        self.feature_names = ['momentum_1m', 'momentum_3m', 'volatility_1m', 'rsi']
    ...
    def train_model(self, price_data, stock_universe, cutoff_date):
        training_start_date = cutoff_date - timedelta(days=365)
        train_price_data = price_data.loc[training_start_date:cutoff_date]
        
        if len(train_price_data) < 126:
            self.is_trained = False
            return False
        ...
        for batch in stock_batches:
            ...
            for stock, features in batch_features:
                ...
                if not full_data.empty and len(full_data) > 20:
...
class AdvancedTradingStrategy:
    ...
    def run_backtest(self, start_date, end_date, selected_stocks=None, transaction_cost_bps=0):
        ...
        fetch_start_date = backtest_start_dt - pd.DateOffset(days=400)
        ...
        rebalance_freq = max(self.rebalance_frequency, 45)
        ...
        for i in range(len(valid_rebalance_dates) - 1):
            ...
            recent_data = ranking_data.iloc[-126:]
