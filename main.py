# main.py - Advanced Risk-Aware Trading System
import pandas as pd
import numpy as np
    ...
    def predict_returns(self, price_data, stocks, as_of_date):
        ...
        for stock in stocks:
            try:
                ...
                ridge_pred, mlp_pred = self.ridge.predict(feature_vector_selected)[0], self.mlp.predict(feature_vector_selected)[0]
                predictions[stock] = (ridge_pred + mlp_pred) / 2
            except Exception:
                continue
        return predictions

class AdvancedTradingStrategy:
    def __init__(self, sharpe_lookback=30, rebalance_frequency=30, max_stocks=20):
        self.sharpe_lookback, self.rebalance_frequency, self.max_stocks = sharpe_lookback, rebalance_frequency, max_stocks
    ...
    def run_backtest(self, start_date, end_date, selected_stocks=None, transaction_cost_bps=0):
        ...
        return {
            ...
            'portfolio_values': portfolio_returns.tolist(), 'benchmark_values': benchmark_returns.tolist(), 'dates': portfolio_returns.index.strftime('%Y-%m-%d').tolist(), 'portfolio_compositions': portfolio_compositions, 
            'performance_metrics': {
                'total_return': float(total_return), 'benchmark_total_return': float(benchmark_total_return),
                'alpha': float(alpha), 'beta': float(beta), 'sortino_ratio': float(sortino_ratio), 'calmar_ratio': float(calmar_ratio),
    ...
    for ticker in tickers:
        try:
            ...
            if info and info.get('marketCap'):
                infos.append({'ticker': ticker, 'market_cap': info.get('marketCap'), 'pe_ratio': info.get('trailingPE'), 'pb_ratio': info.get('priceToBook'), 'dividend_yield': info.get('dividendYield'), 'ev_ebitda': info.get('enterpriseToEbitda'), 'roe': info.get('returnOnEquity')})
...
def get_nifty50_stocks():
    ...
    stock_infos.sort(key=lambda x: x.get(sort_by) if x.get(sort_by) is not None else (float('-inf') if reverse_sort else float('inf')), reverse=reverse_sort)
...
def run_backtest_api():
    try:
        ...
        results = strategy.run_backtest(data.get('start_date'), data.get('end_date'), data.get('selected_stocks'), transaction_cost_bps)
